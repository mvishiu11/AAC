...
We say that ( G ) is a **subgraph** of ( H ) (written ( G \subseteq H ))
...
[
m_G(v_i, v_j) \le m_H(\phi(v_i), \phi(v_j))
]
...
Check if ( G \subseteq H ), i.e., whether ( G ) can be embedded into ( H ).
...
| Refinement          | Local consistency checks       | (O(                 | V_G                       | ^2 | V_H | ^2)) worst | OK for small graphs    |
...
| Subgraph check (G ⊆ H)                         | ✅              | Full implementation with multiplicity-aware Ullmann algorithm.                     |   |   |   |                                               |
